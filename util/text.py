...
def read_csvs(csv_files):
    ...
    for csv in csv_files:
        file = pd.read_csv(csv, encoding='utf-8', na_filter=False)
        #FIXME: not cross-platform
        sets.append(file)
    # Concat all sets, drop any extra columns, re-index the final result as 0..N
    return pd.concat(sets, join='inner', ignore_index=True)



def loading_rdata(files, eng=True, num=True, punc=False):
